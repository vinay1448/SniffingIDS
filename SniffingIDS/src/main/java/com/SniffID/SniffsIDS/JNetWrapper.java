...
public class JNetWrapper {
    ...
    static int snaplen = 64 * 1024;          
    static int flags = Pcap.MODE_PROMISCUOUS; 
    ...
    public static void selectDevice(int i) {
        if (i >= 0 && i < alldevs.size()) {
            selectedDevice = alldevs.get(i);
            sendToConsole("Selected device: " + selectedDevice.getName());
        } else {
            sendToConsole("Invalid device index: " + i);
        }
    }
